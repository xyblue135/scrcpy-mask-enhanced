...
use crate::{
    mask::mapping::{
        ...
        movement_assist::{MovementAssistState, cancel_all_stealth},
...
pub struct BindMappingDirectionPad {
    ...
    pub toggle_run_mode: bool,
    pub toggle_run_enabled: bool,
...
impl From<MappingDirectionPad> for BindMappingDirectionPad {
    fn from(value: MappingDirectionPad) -> Self {
        Self {
            ...
            toggle_run_mode: value.toggle_run_mode,
            toggle_run_enabled: value.toggle_run_enabled,
    ...
    if let Some(active_mapping) = &active_mapping.0 {
        for (action, mapping) in &active_mapping.mappings {
            if action.as_ref().starts_with("DirectionPad") {
                ...
                let raw_state = ineffable.direction_2d(action.ineff_dual_axis());

                let toggle_run_available = mapping.toggle_run_mode
                    && mapping.toggle_run_enabled
                    && mapping.up_boost_key.is_some();
                let boost_just_pressed = mapping.up_boost_key.as_ref().is_some_and(|b| {
                ...
                let boost_held = mapping.up_boost_key.as_ref().is_some_and(|b| {
                    b.is_any_key_pressed(&key_input) || b.is_any_mouse_pressed(&mouse_input)
                });
                let boost_latched = toggle_run_available
                    && movement_assist.toggle_run_actions.contains(&key);
                // In toggle-run mode, the boost key is an edge-triggered toggle: pressing
                // it once latches sprint, pressing it again stops sprint immediately.
                // If the special mode is disabled, keep the original hold-to-boost behavior.
                let boost_active = if toggle_run_available { boost_latched } else { boost_held };
                if state.y < 0.0 && boost_active {
