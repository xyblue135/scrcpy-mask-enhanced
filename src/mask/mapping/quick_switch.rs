...
pub struct PendingQuickSwitchLoad {
    ...
    loading: bool,
    /// 后台加载完成的结果（目标文件, 结果）。
    done: Option<(String, Result<(crate::mask::mapping::config::BindMappingConfig, bevy_ineffable::config::InputConfig), String>)>,
}
...
pub fn handle_mapping_quick_switch(
    ...
    runtime.spawn_background_task(move |mut ctx| async move {
        ...
        let result = match tokio::task::spawn_blocking(move || load_mapping_config(&load_file)).await
        {
            Ok(value) => value,
