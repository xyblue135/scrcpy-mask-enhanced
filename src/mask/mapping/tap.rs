...
use crate::{
    mask::mapping::{
        ...
        movement_assist::{MovementAssistState, cancel_all_stealth, send_stealth_toggle},
...
pub fn cleanup_tap_on_stop(
    ...
    mut movement_assist: ResMut<MovementAssistState>,
) {
    if let Some(active_mapping) = &active_mapping.0 {
        cancel_all_stealth(&mut movement_assist, active_mapping, &cs_tx_res);
    ...
    movement_assist.stealth_actions.clear();
}
...
fn default_empty_cancel_bind() -> ButtonBinding {
    ButtonBinding::new(Vec::new())
}
...
pub fn handle_single_tap(
    ...
    key_input: Res<bevy::input::ButtonInput<bevy::input::keyboard::KeyCode>>,
        ...
        for (action, mapping) in &active_mapping.mappings {
            if action.as_ref().starts_with("SingleTap") {
                ...
                if mapping.stealth_mode {
                    ...
                    let toggled_by_stealth_key = ineffable.just_activated(action.ineff_continuous());

                    if toggled_by_stealth_key {
                        if movement_assist.stealth_actions.remove(&key) {
                            send_stealth_toggle(&cs_tx_res, mapping, original_size);
                            log::debug!("[MovementAssist] stealth off: {}", mapping.id);
                        } else {
                    ...
                    let cancel_pressed = mapping.cancel_bind.is_any_key_just_pressed(&key_input)
                        || mapping.cancel_bind.is_any_mouse_just_pressed(&mouse_input);
                    if cancel_pressed && movement_assist.stealth_actions.remove(&key) {
                        // The game-side stealth button is a toggle, so cancellation is
                        // implemented exactly as requested: tap the stealth coordinate again.
                        send_stealth_toggle(&cs_tx_res, mapping, original_size);
                        log::debug!("[MovementAssist] stealth cancelled by movement key: {}", mapping.id);
                    }
                    continue;
                }
